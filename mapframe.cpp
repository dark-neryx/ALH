          ...
          :CAhFrame (parent, "", wxDEFAULT_FRAME_STYLE | wxCLIP_CHILDREN )
{
	...
	OutputDebugString((LPCWSTR)"Testing5");
    ...
    MakeMenu(layout);
	OutputDebugString((LPCWSTR)"Testing6");

    MakeToolBar();
	OutputDebugString((LPCWSTR)"Testing7");
...
void CMapFrame::MakeMenu(int layout)
{
    ...
    wxMenu    * menuItem;
//    wxMenu    * menuSubItem;
	OutputDebugString((LPCWSTR)"Testing8");


    menuBar = new wxMenuBar();
	OutputDebugString((LPCWSTR)"Testing9");
...
#define TOOLCOUNT  13
	...
	OutputDebugString((LPCWSTR)"Testing10");
    ...
    for (i=0; i<TOOLCOUNT; i++)
        toolBar->AddTool(tool_zoomin+i, toolTip[i], toolBarBitmap[i]);
